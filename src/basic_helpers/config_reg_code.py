...
from typing import Sequence, Union, Tuple, TypedDict, Set
...
BASE_CELLS_LIST: Sequence[Coordinate] = [(lat, lon) for lat, lon in 
                                         product(range(base_bbox[1], base_bbox[3]), 
                                                 range(base_bbox[0], base_bbox[2]+1), repeat=1)]
AZOR_CELLS_LIST: Sequence[Coordinate] = [(lat, lon) for lat, lon in 
                                         product(range(az_bbox[1], az_bbox[3]), 
                                                 range(az_bbox[0], az_bbox[2]+1), repeat=1)]
BASE_CELLS_LIST: Sequence[Coordinate] = [(lat, lon) for lat, lon in 
                                         product(range(base_bbox[1], base_bbox[3]), 
                                                 range(base_bbox[0], base_bbox[2]+1), repeat=1)]
MADE_CELLS_LIST: Sequence[Coordinate] = [(lat, lon) for lat, lon in 
                                         product(range(mad_bbox[1], mad_bbox[3]), 
                                                 range(mad_bbox[0], mad_bbox[2]+1), repeat=1)]
CANA_CELLS_LIST: Sequence[Coordinate] = [(lat, lon) for lat, lon in 
                                         product(range(can_bbox[1], can_bbox[3]), 
                                                 range(can_bbox[0], can_bbox[2]+1), repeat=1)]
CABO_CELLS_LIST: Sequence[Coordinate] = [(lat, lon) for lat, lon in 
                                         product(range(cpv_bbox[1], cpv_bbox[3]), 
                                                 range(cpv_bbox[0], cpv_bbox[2]+1), repeat=1)]
ICEL_CELLS_LIST: Sequence[Coordinate] = [(lat, lon) for lat, lon in 
                                         product(range(isl_bbox[1], isl_bbox[3]), 
                                                 range(isl_bbox[0], isl_bbox[2]+1), repeat=1)]
MEDI_CELLS_LIST: Sequence[Coordinate] = [(lat, lon) for lat, lon in 
                                         product(range(med_bbox[1], med_bbox[3]), 
                                                 range(med_bbox[0], med_bbox[2]+1), repeat=1)]

TOTAL_CELLS: Set[Coordinate] = set(BASE_CELLS_LIST + AZOR_CELLS_LIST + MADE_CELLS_LIST + 
                                   CANA_CELLS_LIST + CABO_CELLS_LIST+ ICEL_CELLS_LIST + MEDI_CELLS_LIST)
